...
const PO_STYLE: Record<POStatus, string> = {
  ...
  ordered: "bg-[rgba(8,145,178,0.12)] text-[var(--color-primary)]",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        {[
          { label: "Stock lines", value: items.length, icon: Package, fg: "text-[var(--color-primary)]", bg: "bg-[rgba(8,145,178,0.07)]" },
          { label: "Reorder alerts", value: low.length, icon: AlertTriangle, fg: "text-amber-600", bg: "bg-amber-50" },
          { label: "Open requests", value: openPOs.length, icon: ShoppingCart, fg: "text-[var(--color-primary)]", bg: "bg-[rgba(8,145,178,0.07)]" },
              ...
              <div key={po.id} className="px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-slate-800 flex items-center gap-2 flex-wrap">
                    ...
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${po.kind === "patient" ? "bg-[rgba(8,145,178,0.12)] text-[var(--color-primary)]" : "bg-[rgba(8,145,178,0.12)] text-[var(--color-primary)]"}`}>{po.kind === "patient" ? "PATIENT" : "RESTOCK"}</span>
                  </p>
      ...
      {low.length > 0 && (
        <div className="rounded-xl border border-[rgba(8,145,178,0.20)] bg-[rgba(8,145,178,0.07)] p-4">
          <p className="flex items-center gap-2 text-sm font-bold text-[var(--color-primary-dark)] mb-1"><Sparkles className="h-4 w-4" /> AI reorder suggestion</p>
          <p className="text-xs text-[var(--color-primary)]">{low.map(i => `${i.name} (${i.qty}/${i.reorderAt})`).join(" · ")} — below reorder level; raise restock requests below.</p>
        </div>
      )}
                    ...
                    {lowStock ? (
                      alreadyRequested ? (
                        ...
                        <button onClick={() => requestRestockFor(i)}
                          className="flex items-center gap-1.5 text-xs font-bold text-[var(--color-primary)] bg-[rgba(8,145,178,0.07)] hover:bg-[rgba(8,145,178,0.14)] px-3 py-1.5 rounded-lg cursor-pointer"><Send className="h-3.5 w-3.5" />Request restock</button>
                      )
                    ) : (
