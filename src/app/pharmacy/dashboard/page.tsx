...
const SOURCE_STYLE: Record<RxSource, string> = {
  OPD: "bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)]", IPD: "bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)]", ICU: "bg-red-50 text-red-700",
  OT: "bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)]", "Home Rx": "bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)]", Discharge: "bg-amber-50 text-amber-700",
}
const STATUS_STYLE: Record<string, string> = {
  queued: "bg-amber-100 text-amber-700", preparing: "bg-[rgba(8,145,178,0.12)] text-[var(--color-primary)]", ready: "bg-green-100 text-green-700",
}
...
export default function PharmacyOverview() {
  ...
  const KPIS = [
    ...
    { label: "On counters", value: m.onCounters, icon: Clock, fg: "text-[var(--color-primary)]", bg: "bg-[rgba(8,145,178,0.07)]" },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
              ...
              <Link href="/pharmacy/queue" className="text-xs font-bold text-[var(--color-primary)] hover:underline flex items-center gap-1">Open queue <ArrowRight className="h-3 w-3" /></Link>
            </div>
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-[rgba(8,145,178,0.20)] bg-[rgba(8,145,178,0.07)] p-4">
            <p className="text-xs font-semibold uppercase tracking-wide text-[var(--color-primary)]">Collected today</p>
            ...
            <p className="text-xs text-[var(--color-primary)] mt-0.5">{m.collectedTodayCount} prescription(s) dispensed</p>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-3">
            ...
            <Link href="/pharmacy/inventory" className="flex items-center justify-between hover:bg-slate-50 -mx-1 px-1 py-1.5 rounded-lg">
              <span className="text-sm text-slate-600 flex items-center gap-2"><ShoppingCart className="h-4 w-4 text-[var(--color-primary)]" /> Open purchase orders</span>
            ...
            <Link href="/pharmacy/inventory" className="flex items-center justify-between hover:bg-slate-50 -mx-1 px-1 py-1.5 rounded-lg">
              <span className="text-sm text-slate-600 flex items-center gap-2"><Package className="h-4 w-4 text-[var(--color-primary)]" /> Stock lines</span>
