...
export default function ReceptionTPA() {
  ...
  const tiles = [
    { label: 'Claims value',    value: `₹${(totalClaimsValue / 100000).toFixed(1)}L`, icon: FileText,     tint: 'bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)]' },
    ...
    { label: 'Active claims',   value: `${claims.length}`,                             icon: ShieldCheck,  tint: 'bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)]' },
  ]
          ...
          <div key={c.id} className="rounded-2xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06),0_4px_16px_rgba(15,23,42,0.04)] p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-3">
                <span className="h-10 w-10 rounded-2xl bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)] flex items-center justify-center flex-shrink-0"><ShieldCheck className="h-5 w-5" /></span>
                <div>
            ...
            {typeof c.aiProbability === 'number' && (
              <div className="mt-3 rounded-xl bg-[rgba(8,145,178,0.07)] border border-[rgba(8,145,178,0.15)] p-2.5">
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-[12px] font-bold text-[var(--color-primary-dark)] flex items-center gap-1.5"><Sparkles className="h-3.5 w-3.5" /> AI approval likelihood</span>
                  <span className="text-[12px] font-bold text-[var(--color-primary)]">{c.aiProbability}%</span>
                </div>
                <div className="h-1.5 rounded-full bg-[rgba(8,145,178,0.12)] overflow-hidden">
