...
const STATUS_STYLE: Record<string, { tint: string; dot: string }> = {
  ...
  Occupied:    { tint: 'bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)] border-[rgba(8,145,178,0.15)]',      dot: 'bg-[rgba(8,145,178,0.07)]0' },
  Cleaning:    { tint: 'bg-amber-50 text-amber-700 border-amber-100',   dot: 'bg-amber-500' },
  Reserved:    { tint: 'bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)] border-[rgba(8,145,178,0.15)]',dot: 'bg-[rgba(8,145,178,0.07)]0' },
...
const ETA_TONE: Record<string, string> = {
  ...
  red: 'bg-red-100 text-red-700', violet: 'bg-[rgba(8,145,178,0.12)] text-[var(--color-primary)]', slate: 'bg-slate-100 text-slate-500',
}
...
export default function ReceptionBeds() {
  ...
  const tiles = [
    ...
    { label: 'Occupied', value: count('Occupied'), icon: User, tint: 'bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)]' },
              ...
              {freeing.map(b => {
                ...
                return (
                  <div key={b.id} className="flex items-center gap-3 rounded-xl bg-slate-50 px-3 py-2.5">
                    <span className={cn("h-9 w-9 rounded-xl flex items-center justify-center flex-shrink-0", b.status === 'Cleaning' ? "bg-amber-100 text-amber-600" : "bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)]")}>
