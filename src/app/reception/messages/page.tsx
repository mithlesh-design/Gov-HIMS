...
const THREAD_STATUS: Record<string, string> = {
  active: 'bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)]', escalated: 'bg-red-50 text-red-600', resolved: 'bg-green-50 text-green-700',
}
    ...
    <div className="pb-6">
      ...
      <div className="flex items-center gap-1 p-1 rounded-xl bg-slate-100 w-fit mb-4">
        ...
        <button onClick={() => setTab('inbox')} className={cn("flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-[13px] font-semibold transition", tab === 'inbox' ? "bg-white text-slate-900 shadow-sm" : "text-slate-500")}>
          <Bell className="h-3.5 w-3.5" /> Team inbox {unreadCount > 0 && <span className="text-[11px] font-bold px-1.5 rounded-full bg-[rgba(8,145,178,0.12)] text-[var(--color-primary)]">{unreadCount}</span>}
        </button>
      </div>
      ...
      {tab === 'chats' ? (
            ...
            {threads.map(t => {
              ...
              return (
                <button key={t.id} onClick={() => setSelectedId(t.id)}
                  className={cn("w-full text-left p-3.5 border-b border-slate-50 transition", selectedId === t.id ? "bg-[rgba(8,145,178,0.07)]/50" : "hover:bg-slate-50")}>
              ...
              <>
                  ...
                  {selected.messages.map(m => {
                    ...
                    return (
                      <div key={m.id} className={cn("flex", mine ? "justify-end" : "justify-start")}>
                        <div className={cn("max-w-[75%] rounded-2xl px-3.5 py-2", mine ? "bg-[var(--color-primary)] text-white" : ai ? "bg-[rgba(8,145,178,0.07)] text-[var(--color-primary-dark)] border border-[rgba(8,145,178,0.15)]" : "bg-white text-slate-800 border border-slate-100")}>
                ...
                <div className="p-3 border-t border-slate-100 flex items-center gap-2">
                  <input value={reply} onChange={e => setReply(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') sendReply() }}
                    placeholder="Reply as front desk…" className="flex-1 h-10 px-3.5 rounded-xl bg-slate-50 border border-slate-200 text-[14px] text-slate-800 outline-none focus:border-[rgba(8,145,178,0.30)] focus:ring-2 focus:ring-blue-100" />
                  <button onClick={sendReply} disabled={!reply.trim()} aria-label="Send" className="h-10 w-10 rounded-xl bg-[var(--color-primary)] disabled:bg-slate-200 text-white flex items-center justify-center active:scale-95 transition"><Send className="h-4 w-4" /></button>
                </div>
              </>
        ...
        <div className="rounded-2xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06),0_4px_16px_rgba(15,23,42,0.04)] p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            {unreadCount > 0 && <button onClick={() => markAllRead()} className="text-[12.5px] font-semibold text-[var(--color-primary)] hover:text-[var(--color-primary)]">Mark all read</button>}
          </div>
          <div className="space-y-2">
            {notifications.map(n => (
              <button key={n.id} onClick={() => markRead(n.id)}
                className={cn("w-full text-left flex items-start gap-3 rounded-xl p-3 transition", n.read ? "bg-white hover:bg-slate-50" : "bg-[rgba(8,145,178,0.07)]/50 hover:bg-[rgba(8,145,178,0.10)]")}>
                {!n.read && <span className="h-2 w-2 rounded-full bg-[rgba(8,145,178,0.07)]0 mt-1.5 flex-shrink-0" />}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
              ...
              <div className="flex items-center justify-between mb-5">
                <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2"><Headset className="h-5 w-5 text-[var(--color-primary)]" /> New message</h2>
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button onClick={sendInternal} className="flex-1 h-11 rounded-xl bg-[var(--color-primary)] hover:bg-[var(--color-primary-dark)] text-white font-bold text-[13.5px] flex items-center justify-center gap-2"><Send className="h-4 w-4" /> Send</button>
              </div>
            </motion.div>
          </motion.div>
        )}
