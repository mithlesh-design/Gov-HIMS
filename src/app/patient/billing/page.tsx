...
function downloadReceipt(admission: { id: string; date: string; title: string; insurer: string; total: number; covered: number }) {
  ...
  const html = `
    ...
    <p class="muted">Tax invoice — system-generated by Agentix HIMS.</p>`
  ...
  return (
    <div className={cn("rounded-3xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06),0_8px_28px_rgba(15,23,42,0.05)] p-5", status === 'pending' && "ring-1 ring-amber-200")}>
      ...
      {status === 'paid' ? (
        ...
        <Link href={payHref ?? '#'} className="mt-3 w-full bg-[var(--color-primary)] text-white font-bold text-[14px] rounded-xl py-2.5 flex items-center justify-center gap-2 active:scale-[0.98] transition">
