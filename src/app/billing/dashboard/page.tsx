...
const STATUS_STYLE: Record<string, { bg: string; border: string; badge: "success" | "warning" | "blue" | "muted" | "danger" }> = {
  ...
  frozen:   { bg: "bg-[rgba(8,145,178,0.07)]",   border: "border-[rgba(8,145,178,0.20)]",   badge: "blue" },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          ...
          { label: "Bills Settled", value: settled, color: "text-[var(--color-primary)]", bg: "bg-[rgba(8,145,178,0.07)] border-[rgba(8,145,178,0.20)]" },
          { label: "AI Duplicate Flags", value: duplicates.reduce((s, d) => s + d.alerts.length, 0), color: "text-[var(--color-primary)]", bg: "bg-[rgba(8,145,178,0.07)] border-[rgba(8,145,178,0.20)]" },
      ...
      {duplicates.length > 0 && (
        <div className="rounded-xl border border-[rgba(8,145,178,0.20)] p-4 bg-gradient-to-br from-[rgba(8,145,178,0.07)] to-[rgba(8,145,178,0.04)]">
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            <div className="flex items-center gap-2">
              <Sparkles className="h-4 w-4 text-[var(--color-primary)]" />
              <h3 className="text-sm font-bold text-[var(--color-primary-dark)]">AI Duplicate-Charge Audit</h3>
              <span className="text-[10px] font-bold uppercase tracking-wide bg-[rgba(8,145,178,0.12)] text-[var(--color-primary)] px-2 py-0.5 rounded">
                ₹{totalDuplicateValue.toLocaleString('en-IN')} flagged
              </span>
            </div>
            <p className="text-[11px] text-[var(--color-primary)]">Recurring items (ward/nursing/consultation) excluded</p>
          </div>
          <div className="space-y-2">
            {duplicates.map(({ bill, alerts }) => (
              <div key={bill.id} className="rounded-lg bg-white border border-[rgba(8,145,178,0.15)] p-3">
                <p className="text-xs font-bold text-slate-800">{bill.patientName} <span className="text-slate-400">· {bill.id}</span></p>
                {alerts.map((a) => (
                  <p key={a.groupKey} className="text-[11px] text-[var(--color-primary)] mt-1 flex items-start gap-1">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-1 flex-wrap">
                      ...
                      {hasDupes && (
                        <span className="text-[10px] font-bold uppercase px-1.5 py-0.5 rounded bg-[rgba(8,145,178,0.12)] text-[var(--color-primary)] flex items-center gap-1">
                    ...
                    <div className="flex items-center gap-6 text-sm flex-wrap">
                      ...
                      {bill.insuranceCovered > 0 && <span className="text-[var(--color-primary)] font-medium">Insurance: ₹{bill.insuranceCovered.toLocaleString('en-IN')}</span>}
                  ...
                  <div className="flex gap-2 items-center">
                    {bill.status === 'draft' && (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-[rgba(8,145,178,0.07)] hover:bg-[rgba(8,145,178,0.14)] text-[var(--color-primary)] text-xs font-bold cursor-pointer"
                      >
