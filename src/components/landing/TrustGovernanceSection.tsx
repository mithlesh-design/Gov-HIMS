    ...
    <section id="security" className="scroll-mt-20 py-20 lg:py-28">
      <div className="max-w-7xl mx-auto px-5 lg:px-10">
        <Reveal className="max-w-2xl">
          <p className="text-[12px] font-bold uppercase tracking-[0.14em] text-[var(--color-primary)]">Trust &amp; governance</p>
          ...
          {PILLARS.map(({ icon: Icon, title, points }, i) => (
            <Reveal key={title} delay={i * 0.06}>
              <div className="rounded-3xl border border-[#EAECF2] bg-white p-5 h-full">
                <span className="h-11 w-11 rounded-2xl flex items-center justify-center bg-[var(--color-primary)]/[0.10] text-[var(--color-primary)]"><Icon className="h-5 w-5" /></span>
                ...
                <ul className="mt-2.5 space-y-1.5">
                  {points.map(p => (
                    <li key={p} className="flex items-start gap-2 text-[12.5px] text-[#475467]">
                      <ShieldCheck className="h-3.5 w-3.5 text-[var(--color-primary)] flex-shrink-0 mt-0.5" /><span>{p}</span>
                    </li>
                  ))}
                </ul>
              </div>
