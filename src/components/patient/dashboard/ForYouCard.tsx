...
function nudgesFor(mode: LiveMode, stage: OpdStage): Nudge[] {
  if (mode === 'video') {
    if (stage === 'booked' || stage === 'waiting_room') return [
      { icon: Video, tint: 'bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)]', title: 'Be ready for your video call', detail: 'Find a quiet, well-lit spot and test your camera & mic.', cta: 'Test camera & mic', priority: true },
      { icon: Upload, tint: 'bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)]', title: 'Add your past reports', detail: 'Share old prescriptions or reports so your doctor is prepared.', cta: 'Upload reports' },
      { icon: Stethoscope, tint: 'bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)]', title: 'Answer a few questions', detail: 'A quick pre-consult helps your doctor make the most of the call.', cta: 'Start pre-consult' },
    ]
    if (stage === 'in_call') return [
      ...
      { icon: FileText, tint: 'bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)]', title: 'Have your reports handy', detail: 'Share any document during the call if your doctor asks.', cta: 'My reports' },
    ]
    return [ // prescription / done
      { icon: FileText, tint: 'bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)]', title: 'e-Prescription ready', detail: 'View your prescription and order medicines for home delivery.', cta: 'View & order', priority: true },
  ...
  if (stage === 'waiting' || stage === 'vitals') return [
    { icon: Activity, tint: 'bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)]', title: "We'll call you for vitals", detail: 'Keep your phone close — your live status updates right here.', cta: 'View live status', priority: true },
    { icon: Upload, tint: 'bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)]', title: 'Add your past reports', detail: 'Share old prescriptions or lab reports with your doctor.', cta: 'Upload reports' },
    { icon: MapPin, tint: 'bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)]', title: 'Find your way', detail: 'Directions to the vitals room and consultation.', cta: 'Hospital map' },
  ]
  if (stage === 'consulting') return [
    ...
    { icon: Upload, tint: 'bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)]', title: 'Add your past reports', detail: 'Share documents your doctor may need.', cta: 'Upload reports' },
  ]
  return [ // pharmacy / billing / done — results now exist
    { icon: FlaskConical, tint: 'bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)]', title: 'Lab result ready — explained', detail: 'Your CBC was reviewed; we put it in plain language.', cta: 'See what it means', priority: true },
    { icon: FileText, tint: 'bg-[rgba(8,145,178,0.07)] text-[var(--color-primary)]', title: 'e-Prescription ready', detail: 'Collect at the pharmacy or order home delivery.', cta: 'View prescription' },
          ...
          return (
            <motion.button
              ...
              className={cn("w-full flex items-center gap-3.5 p-3.5 rounded-2xl text-left transition-all active:scale-[0.99] hover:shadow-sm",
                n.priority ? "bg-[rgba(8,145,178,0.07)]/60 ring-1 ring-blue-100" : "bg-slate-50")}
            >
              ...
              <span className="flex-1 min-w-0">
                ...
                <span className="inline-block text-[12.5px] font-semibold text-[var(--color-primary)] mt-1.5">{n.cta} →</span>
              </span>
